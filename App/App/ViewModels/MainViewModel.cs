...
public partial class MainViewModel(AppConfig appConfig) : ViewModelBase
{
    ...
    [ObservableProperty] private bool _isDocumentLoaded;

    [ObservableProperty] private int _pageCount = 0;
    ...
    public async Task<ExtractionResponse> ExtractAsync()
    {
        ...
        var request = new ExtractionRequest("pdf", "Loan Application", ToBase64String(CurrentDoc!));

        var requestJson = JsonContent.Create(request);
        var httpResponse = await Client.PostAsync(EXTRACT_API, requestJson);
        var content = await httpResponse.Content.ReadAsStringAsync();
        var response = JsonSerializer.Deserialize<ExtractionResponse>(content);
        return response ??
               new ExtractionResponse(
                   null,
                   new ExtractionError("Unknown error", ErrorType.UnknownError),
                   DateTime.Now);
    }
